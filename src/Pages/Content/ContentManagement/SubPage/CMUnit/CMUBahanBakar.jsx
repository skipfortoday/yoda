...
export default function CMUBahanBakar(props) {
  ...
  async function LoadData() {
    await axiosBackend.get('/cm/bahan-bakar')
    .then((response) => { 
  ...
  async function InsertData() {
    await axiosBackend.post('/cm/bahan-bakar', {
  ...
  const DATAGRID_COLUMNS = [
    ...
    { field: 'bahan_bakar', headerName: 'Jenis bahan bakar', minWidth: 180, flex: 1 },
  ]
    ...
    <>
      { indexPage!==ActiveSubPage?null:(
        ...
        >
          ...
          >
            <FormControl variant="outlined" color="primary" fullWidth error={InputBahanBakar.error}>
              <InputLabel htmlFor="input-1">{InputBahanBakar.label}</InputLabel>
              <OutlinedInput
                id="input-1"
                type="text"
                value={InputBahanBakar.value}
                onChange={(e) => setInputBahanBakar({...InputBahanBakar, value: e.target.value})}
