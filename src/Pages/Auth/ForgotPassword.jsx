...
import React, { useState } from 'react'
import { Box } from '@mui/system';
import { Button, FormControl, Grid, IconButton, InputAdornment, InputLabel, OutlinedInput, Stack, Typography, Collapse, Popper, Fade, Paper } from '@mui/material';
import { useTheme } from '@mui/material/styles';
import useMediaQuery from '@mui/material/useMediaQuery';
import AuthPageBannerCard from '../../Components/Auth/AuthPageBannerCard';
import { useHistory } from 'react-router-dom';
...
const TEXTS = {
  ...
  form1: {
    header1: 'Lupa kata sandi?',
    header2: 'Harap masukkan email Anda. Kami akan mengirimkan link untuk mengubah kata sandi melalui email Anda.',
    email: 'Email',
    submitButton: 'Lanjutkan'
  },
  form2: {
    header1: 'Cek email Anda!',
    header2: 'Kami sudah mengirimkan link agar Anda dapat mengubah kata sandi Anda.',
    submitButton: 'Kembali ke halaman login'
  }
}
...
export default function ForgotPasswordPage() {
  ...
  async function handleForgot() {
    await axiosBackend.post('/forgot-password',{
      email: InputEmail.value,
    })
    .then((response) =>{ 
      setInputEmail({...InputEmail, error: false})
            ...
            <Collapse in={ActiveSection===0} timeout="auto">
              ...
              <Stack direction="column" sx={{ paddingX: upLg?12:upMd?4:2 }} alignItems="center" spacing={1.5}>
                <FormControl variant="outlined" color="primary" fullWidth
                  error={InputEmail.error} id="formEmail"
                >
                  ...
                  <OutlinedInput
                    ...
                    onChange={(e) => {
                      ...
                      handleClick("right-end", formBorder)
                    }}
                    endAdornment={
                      <InputAdornment position="end">
                ...
                <FormControl fullWidth sx={{ paddingTop: 1.5 }}>
                  <Button variant="contained" color="primary" size="large" disabled={!EmailFormat} onClick={handleNextClick}>{TEXTS.form1.submitButton}</Button>
                </FormControl>
              </Stack>
            </Collapse>
            <Collapse in={ActiveSection===1} timeout="auto">
              <Stack direction="column" sx={{ paddingY: 4, marginBottom: upMd?2:2 }} alignItems="center" spacing={2}>
                <img src="./images/web/success.png" alt="Success" width={upMd?"200px":"200px"} />
                <Typography variant="div" fontWeight="bold" fontSize={36} sx={{  }}>{TEXTS.form2.header1}</Typography>
                <Typography variant="div" fontSize={14} color="text.secondary" sx={{ width: 420, textAlign: 'center' }}>{TEXTS.form2.header2}</Typography>
              </Stack>
              <Stack direction="column" sx={{ paddingX: upLg?12:upMd?4:2 }} alignItems="center" spacing={1.5}>
                <FormControl fullWidth sx={{ paddingTop: 1.5 }}>
                  <Button variant="contained" color="primary" size="large" onClick={handleToLoginClick}>{TEXTS.form2.submitButton}</Button>
                </FormControl>
              </Stack>
            </Collapse>
          </Box>
